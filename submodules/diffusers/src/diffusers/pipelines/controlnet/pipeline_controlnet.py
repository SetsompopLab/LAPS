...
from ..stable_diffusion.data_consistency_utils import(
...
class StableDiffusionControlNetPipeline(
    ...
    def __init__(
        ...
        # DIFF NOTE: StableDiffusionPipeline uses do_convert_rgb=False, but we use do_convert_rgb=True
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # NOTE: StableDiffusionPipeline defaults height and width to unet here, whereas SDCPipeline does not
            # default height, width: self.unet.config.sample_size * self.vae_scale_factor according to docs.
            # but code might be different... i'll set this here like SD does for now
        ...
        if measurements is not None:
            ...
            use_dc_caching = kwargs.get("dc_caching", False)
            ...
            for i, t in enumerate(timesteps):
                ...
                if do_dc and dc_type == "hard":
                    if (t in image_dc_timesteps) or (t in latent_dc_timesteps):
                        ...
                        z_0_hat = (latents - beta_prod_t ** (0.5) * noise_pred) / alpha_prod_t ** (0.5)
                        ...
                        if not debug_dc:
                            if t in latent_dc_timesteps:
                                ...
                                z_0_opt = dc_latent_optimization(forward_model,
                                                                    self.vae,
                                                                    opt_params["latent"],
                                                                    measurements,
                                                                    z_0_hat,
                ...
                if do_dc and dc_type == "manifold_ldps":
                    if (t in latent_dc_timesteps):
                        ...
                        alpha_prod_t = self.scheduler.alphas_cumprod[t]
                        beta_prod_t = 1 - alpha_prod_t
                        latents = dc_latent_manifold_optimization(
                            forward_model,
                            self.vae,
                            self.unet,
                            opt_params["latent"],
                ...
                if do_dc and dc_type == "ldps":
                    if t in latent_dc_timesteps:
                        ...
                        z_0_opt = dc_latent_optimization(forward_model,
                                                            ...
                                                            z_0_hat,
                                                            generator=generator)
                        latents = z_0_opt * alpha_prod_t ** (0.5) + beta_prod_t ** (0.5) * noise_pred
                #### End of added code for DC ####
