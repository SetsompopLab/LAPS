...
from ..attention_processor import Attention, AttnAddedKVProcessor, AttnAddedKVProcessor2_0, AttnProcessor2_0, AttnProcessor
...
from ..transformers.transformer_2d import Transformer2DModel, Transformer2DSelfAttentionModel
...
def get_down_block(
    ...
    dropout: float = 0.0,
    use_transformers_for_self_attention_blocks: bool = False, # for SelfAttn stuff
):
    ...
    if down_block_type == "DownBlock2D":
        ...
        return SelfAttnDownBlock2D(
            ...
            # for transformer-based self attention
            transformer_attentions=use_transformers_for_self_attention_blocks,
...
class UNetMidBlock2DSelfAttn(nn.Module):
    def __init__(
        ...
        output_scale_factor: float = 1.0,
        # for transformer-based self attention
        transformer_attentions: bool = False,
        transformer_layers_per_block: Union[int, Tuple[int]] = 1,
        use_linear_projection: bool = False,
        upcast_attention: bool = False,
        attention_type: str = "default",
        ...
        for i in range(num_layers):
            ...
            if transformer_attentions:
                attn = Transformer2DSelfAttentionModel(
                    num_attention_heads,
                    in_channels // num_attention_heads,
                    in_channels = in_channels,
                    num_layers = transformer_layers_per_block[i],
                ...
                attn = Attention(
                        ...
                        norm_num_groups=resnet_groups,
                        dropout=dropout,
                        bias=False, # because this is default in BasicTransformerBlock
                        out_bias=True, # because this is default in BasicTransformerBlock
                        upcast_attention=False,
                        residual_connection=True, # to mimic a transformer block in simple way
                    )
    ...
    def forward(
        ...
        for i, (resnet, attn) in enumerate(blocks):
            ...
            if self.training and self.gradient_checkpointing:
                ...
                hidden_states = attn(hidden_states, attention_mask=attention_mask, **cross_attention_kwargs)
...
class SelfAttnUpBlock2D(nn.Module):
    ...
    def forward(
        ...
        if self.upsamplers is not None:
            for upsampler in self.upsamplers:
                if self.upsample_type == "resnet":
                    hidden_states = upsampler(hidden_states, temb=temb, output_size=upsample_size, scale=lora_scale)
                else:
                    hidden_states = upsampler(hidden_states, output_size=upsample_size, scale=lora_scale)
        return hidden_states
