...
class UNet2DLoadersMixin:
    """
    Load LoRA layers into a [`UNet2DModel`].
    """
    ...
    def load_attn_procs(self, pretrained_model_name_or_path_or_dict: Union[str, Dict[str, torch.Tensor]], **kwargs):
        r"""
        Load pretrained attention processor layers into [`UNet2DModel`]. Attention processor layers have to be
        defined in
        [`attention_processor.py`](https://github.com/huggingface/diffusers/blob/main/src/diffusers/models/attention_processor.py)
        and be a `torch.nn.Module` class.
    ...
    def _convert_ip_adapter_attn_to_diffusers(self, state_dicts):
        ...
        key_id = 1
        for name in self.attn_processors.keys():
            # NOTE: only difference between UNEt2DConditionalLoadersMixin and UNet2DLoadersMixin is the `cross_attention_dim` being none for transformer attn2 layers
            cross_attention_dim = None #if name.endswith("attn1.processor") else self.config.cross_attention_dim
            if name.startswith("mid_block"):
                hidden_size = self.config.block_out_channels[-1]
            elif name.startswith("up_blocks"):
