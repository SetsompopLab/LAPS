...
def main(cfg: DictConfig):
    ...
    options = {
        ...
        "log_img_every_n_steps": cfg["log_img_every_n_steps"],
        "val_log_img_every_n_steps": cfg["val_log_img_every_n_steps"],
    }
    print(f"=> Starting model training [epochs={cfg['max_epoch']}]")
    min_loss = None
    global_step = cfg.get("global_step", start_epoch * len(train_dataloader))
    for epoch in range(start_epoch, cfg["max_epoch"]):
        ...
        if min_loss is None or loss < min_loss:
            try:
                best_ckpt_dir = os.path.join(cfg.ckpt_dir, "best")
                os.makedirs(best_ckpt_dir, exist_ok=True)
                accelerator.save_state(best_ckpt_dir, safe_serialization=False)
        ...
        if (epoch + 1) % cfg.get("ckpt_every_n_epochs", 1) == 0:
            ...
            try:
                epoch_ckpt_dir = os.path.join(cfg.ckpt_dir, f"epoch-{epoch:04d}")
                os.makedirs(epoch_ckpt_dir, exist_ok=True)
                accelerator.save_state(epoch_ckpt_dir, safe_serialization=False)
            except Exception as e:
                print(e)
