...
class GenericDataset(Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        # Generate image paths, appending suffix only if missing
        if img_column is not None:
            ...
            for x in self.df.get_column(img_column).to_list():
                ...
                self.samples["img"].append(os.path.join(self.img_dir, file_name))
        ...
        # Extract the column with masks, appending suffix only if missing
    ...
    def __getitem__(self, idx: int):
        ...
        if "img" in self.samples:
            ...
            sample["img_path"] = self.samples["img"][idx]
