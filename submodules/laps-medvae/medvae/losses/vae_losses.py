...
import open_clip
from medvae.utils.globals import QUALITY_MAX, QUALITY_RANGE
...
class LPIPSWithDiscriminator(nn.Module):
    ...
    def forward(
        ...
        # Compute quality-based weights if quality is provided
        if quality is not None:
            # This ensures low quality images contribute less to the loss
            quality_weights = 10.0 ** ((quality.float() - QUALITY_MAX) / QUALITY_RANGE)
            quality_weights = quality_weights.to(inputs.device)
        else:
            # If no quality provided, use uniform weights
            quality_weights = torch.ones(bsz, device=inputs.device)
        ...
        if optimizer_idx == 0:
            ...
            # Apply quality weighting to reconstruction loss
            rec_loss_weighted = rec_loss * quality_weights.view(-1, *([1] * (rec_loss.dim() - 1)))
            nll_loss = (rec_loss_weighted / torch.exp(self.logvar) + self.logvar).sum() / bsz
            ...
            if self.use_biomedclip_loss:
                ...
                # Apply quality weighting to BiomedCLIP loss
                bc_loss_weighted = bc_loss * quality_weights
                bc_loss = bc_loss_weighted.sum() / bsz

            # KL regularization loss
            kl_loss = posteriors.kl()
            # Apply quality weighting to KL loss
            kl_loss_weighted = kl_loss * quality_weights.view(-1, *([1] * (kl_loss.dim() - 1)))
            kl_loss = kl_loss_weighted.sum() / bsz
            ...
            if d_valid:
                ...
                # Apply quality weighting to generator loss
                g_loss_per_sample = -logits_fake.view(bsz, -1).mean(dim=1)
                g_loss_weighted = g_loss_per_sample * quality_weights
                g_loss = g_loss_weighted.mean()
            ...
            log = {
                ...
                f"{split}/quality_weights_mean": quality_weights.detach().mean(),
            }
            if self.use_biomedclip_loss:
            ...
            if d_valid:
                ...
                # Apply quality weighting to discriminator loss
                # Since hinge_loss doesn't support reduction='none', we need to compute it manually
                loss_real = torch.nn.functional.relu(1.0 - logits_real)
                loss_fake = torch.nn.functional.relu(1.0 + logits_fake)
                
                # Apply quality weighting per sample
                loss_real_weighted = loss_real.view(bsz, -1).mean(dim=1) * quality_weights
                loss_fake_weighted = loss_fake.view(bsz, -1).mean(dim=1) * quality_weights
                
                d_loss = d_valid * 0.5 * (loss_real_weighted.mean() + loss_fake_weighted.mean())
